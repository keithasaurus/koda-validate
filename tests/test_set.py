...
def test_set_validator() -> None:
    ...
    assert set_str_v(None) == Invalid(set_str_v, TypeErr(set))
    assert set_str_v({"cool", "neat"}) == Valid({"cool", "neat"})
    assert set_str_v({"bad", 1}) == Invalid(
        set_str_v, SetErrs([Invalid(set_str_v.item_validator, TypeErr(str))])
    )
    ...
    assert set_int_v_2({0, 2}) == Invalid(set_int_v_2, PredicateErrs([MaxItems(2)]))

    set_none_v = SetValidator(BasicNoneValidator())

    assert set_none_v({None}) == Valid({None})
    assert set_none_v({1}) == Invalid(
        set_none_v, SetErrs([Invalid(set_none_v.item_validator, TypeErr(type(None)))])
    )
...
async def test_set_validator_async() -> None:
    ...
    assert await set_str_v.validate_async(None) == Invalid(set_str_v, TypeErr(set))
    ...
    assert await set_int_v_2.validate_async({0, 2}) == Invalid(
        set_int_v_2, PredicateErrs([MaxItems(2)])
    )
    ...
    assert await set_async_v.validate_async({1, 2, 3}) == Invalid(
        set_async_v, PredicateErrs([AsyncSetPred()])
    )
