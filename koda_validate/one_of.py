...
from koda_validate.base import InvalidVariants, Validated, Validator
...
class OneOf2(Validator[Either[A, B]]):
    ...
    async def validate_async(self, val: Any) -> Validated[Either[A, B]]:
    ...
    def __call__(self, val: Any) -> Validated[Either[A, B]]:
...
class OneOf3(Validator[Either3[A, B, C]]):
    ...
    async def validate_async(self, val: Any) -> Validated[Either3[A, B, C]]:
        if (v1_result := self.variant_1(val)).is_valid:
            return Valid(First(v1_result.val))
        else:
            if (v2_result := self.variant_2(val)).is_valid:
    ...
    def __call__(self, val: Any) -> Validated[Either3[A, B, C]]:
        if (v1_result := self.variant_1(val)).is_valid:
            return Valid(First(v1_result.val))
        else:
            if (v2_result := self.variant_2(val)).is_valid:
